...
def copy_anomalies():
    ...
    while True:
        ...
        for patient_id in patients_with_anomalies:
            ...
            cursor.execute("SELECT MIN(timestamp), MAX(timestamp) FROM patients WHERE id = ? AND anomaly = 1", (patient_id[0],))
            ...
            cursor.execute("INSERT INTO anomalies SELECT * FROM patients WHERE id = ? AND timestamp >= ? AND timestamp <= ? AND anomaly = 1", (patient_id[0], start_time, end_time))
            conn.commit()


# Pobieranie aktualnych wartości sensorów dla wybranego pacjenta (do customowego widgetu)
def get_latest_sensor_values(patient_id):
    # Połącz się z bazą danych
    conn = sqlite3.connect('patients.db')
    cursor = conn.cursor()
    ...
    for row in sensor_values:
        sensor_values_list.append({'sensor_name': row[0], 'sensor_value': row[1], 'anomaly': row[2], 'timestamp': row[3]})
...
def get_patient_list():
    ...
    cursor.execute("SELECT lastname, firstname, birthdate, disabled, SUM(anomaly) FROM patients GROUP BY lastname, firstname, birthdate, disabled")
...
def update_graph_live(n, chosen_patient, checked_sensor):
    ...
    if chosen_patient is not None:
        ...
        for s in checked_sensor:
            ...
            x_anomalies = [sensor['timestamp'] for sensor in sensor_list if sensor['sensor_name'] == s and sensor['anomaly'] == 1]
            y = [sensor['sensor_value'] for sensor in sensor_list if sensor['sensor_name'] == s]
            y_anomalies = [sensor['sensor_value'] for sensor in sensor_list if sensor['sensor_name'] == s and sensor['anomaly'] == 1]
